...
The fuzzing session begins in the `run_evolutionary_loop` method. The first step is to choose a parent test case to serve as the genetic material for this session's mutations.
  ...
  * **Scoring:** The orchestrator calls upon the `CorpusManager`, which uses a `CorpusScheduler` to calculate a "fuzzing score" for every file in the corpus. This score is a floating-point number derived from several heuristics:
      ...
      * **Coldness:** Files whose recent mutations all score exactly `0.0` (no new coverage, timing, or JIT-vitals signal — no gradient to climb) accumulate a `consecutive_zero_score` streak. Once that streak passes a threshold, the scheduler applies a graduated penalty so the cold parent is selected less often, and once it exceeds `COLD_STERILITY_LIMIT` (100) the parent is retired (marked sterile) early — far sooner than the blunt `CORPUS_STERILITY_LIMIT` (599) that counts *all* non-finds. A single warm near-miss (score > 0) or a find resets the streak, so parents that still show any signal keep the full leash.
