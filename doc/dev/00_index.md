...
| **[09\_diagnostic\_mode.md](./09_diagnostic_mode.md)** | How to use diagnostic mode for smoke tests, mutator development, and debugging. Covers all `--max-sessions`, `--dry-run`, `--mutators`, and related options. | Developers running verification tests or developing new mutators. |
| **[10\_glossary.md](./10_glossary.md)** | A comprehensive glossary of terminology, concepts, and jargon used throughout the lafleur project. | **All developers.** A quick reference when encountering unfamiliar terms. |
| **[11\_seeding.md](./11_seeding.md)** | How `lafleur` generates its initial corpus using `fusil`, the seeding workflow, and best practices for hand-rolling custom seeds. | Developers and operators wanting to bootstrap campaigns or target specific JIT behaviors. |