...
* **`per_file_coverage`**: A dictionary where each key is a filename in the corpus (e.g., `"123.py"`) and the value is a rich metadata object. The metadata for each file contains:
    ...
    * `consecutive_zero_score`: Counter of consecutive *stone-cold* children — those scoring exactly `0.0` (no new coverage, timing, or JIT-vitals signal, i.e. no gradient for the loop to climb). A warm near-miss (score > 0) or a find resets it to `0`; a non-measurement (duplicate / unparseable / resource-killed child, score `None`) leaves it untouched. When it exceeds `COLD_STERILITY_LIMIT` (100) the file is retired early, well before the blunt `CORPUS_STERILITY_LIMIT`. The corpus scheduler also applies a graduated penalty (`COLDNESS_PENALTY_*`) once the streak passes its threshold, deprioritizing cold parents before retirement.
    ...
    * `is_sterile`: Boolean flag set to `True` when the file has been mutated many times without producing new discoveries (either limit: `CORPUS_STERILITY_LIMIT` or the cold-streak `COLD_STERILITY_LIMIT`). Sterile files receive a heavy penalty in the corpus scheduler and are filtered out of parent selection.
