...
- Extracted `_HTML_STYLE` and `_HTML_SCRIPT` module-level constants plus `_build_html_instance_rows` and `_build_html_crash_rows` helpers from the 461-line `generate_html_report` in `campaign.py`, by @devdanzin.
