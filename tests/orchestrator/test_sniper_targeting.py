...
class TestSniperTargeting(unittest.TestCase):
    def test_harness_exclusion(self):
        ...
        # Mock AST retrieval
        harness_node = ast.FunctionDef(name="uop_harness_test", args=ast.arguments(), body=[ast.Pass()], decorator_list=[])
        setup_nodes = []
        ...
        with patch.object(orch, "_get_nodes_from_parent", return_value=(harness_node, parent_tree, setup_nodes)), \
             patch.object(orch, "_get_mutated_harness", return_value=(None, None)) as mock_get_mutated, \
            ...
            orch.execute_mutation_and_analysis_cycle(
                mock_path, 100.0, 1, False
            )
