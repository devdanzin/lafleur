...
- `scripts/analyze_cold_streaks.py`: a log-analysis tool that replays a `deep_fuzzer_run_*.log` as a per-parent stream of child outcomes and reports cold/warm/find breakdowns, what terminates each stone-cold run, the cold-streak distribution preceding finds, and a retirement-policy simulation across several `K` values — for re-tuning `COLD_STERILITY_LIMIT` against real runs, by @devdanzin.
- A synthesize seed family (`lafleur/jit_seeds.py`): a compact, self-contained `ast`-based grammar (assignments, aug-assignments, binops, comparisons, `if`/`for` nesting) wrapped in a JIT-warming hot loop, for structural / data-flow diversity. Added to `generate_jit_seed`'s family dispatch (`uop`, `bug_pattern`, `synthesize`). Self-contained by design — fusil's `ASTPatternGenerator` grammar is module-coupled, so this re-implements an equivalent grammar natively, by @devdanzin.
...
- Added 9 tests for `corpus_analysis.py` (previously 0) covering `calculate_distribution` and `generate_corpus_stats` with empty/single/multi-file corpora, tree topology, sterile/pruned file handling, and mutation tracking, by @devdanzin.
- Expanded `registry.py` tests from 7 to 30, covering duplicate sighting handling, triage status changes, notes, get_new/triaged/all_crashes, stats, sightings with limits, unlink, nonexistent lookups, and record_issue updates, by @devdanzin.
...
- Added 21 tests for `ScoringManager.find_new_coverage` (10 tests) and `score_and_decide_interestingness` (11 tests), covering global/relative/seed paths, harness aggregation, threshold boundaries, timing mode, and JIT vitals forwarding, by @devdanzin.
- Strengthened 7 hollow test assertions: 4 in `test_jit_tuner.py` now verify stdout messages and file state, 3 in `test_crash_detection.py` now verify stderr messages and `shutil.copy` calls, by @devdanzin.
...
- Fixed stale `telemetry.py` module references in architecture overview and glossary (now `artifacts.py`), corrected "The Strategist" → "The Alchemist" nickname for `mutation_controller.py` in CLAUDE.md, and added docstrings to `CrashType`, `CrashSignature`, and `to_dict()` in `analysis.py`, by @devdanzin.
...
- The session driver (`lafleur/driver.py`, used by `--session-fuzz`) now seeds its shared globals with the runtime dependencies the stripped boilerplate normally supplies (`sys`, `gc`, `collect`, `fuzzer_rng`, `random`). Corpus files are stored core-only, so session parents/polluters were executed without `import sys` — and their first line is the `[fN]` marker `print(..., file=sys.stderr)` — so they raised `NameError: name 'sys' is not defined` before the boilerplate-carrying child ran and defined `sys` in the shared namespace. A real session run wasted ~99.8% of its ignored-crashes on this (only the ~15% child-only "solo" sessions were unaffected), defeating the warm-JIT purpose of session mode. The earlier boilerplate fixes (#867/#878) only covered the normal per-script path; this extends the same guarantee to the driver. By @devdanzin.
- `CrashSignature.to_dict()` now serializes `crash_type` as its plain string value (`.value`) instead of leaving it as a `CrashType(str, Enum)` member. `json.dumps` of a str-Enum subclass is version-dependent — some CPython builds emit the value (`"C_ASSERTION"`), others `str(enum)` (`"CrashType.C_ASSERTION"`) — which made `test_to_dict_json_roundtrip` (and crash metadata.json output) unstable across interpreters and broke CI. By @devdanzin.
- Fixed all 25 pre-existing `mypy lafleur` errors so the static-analysis CI job passes: telemetry/dynamic keys are written through plain-dict views rather than the `RunStats` TypedDict (`artifacts.py`, `orchestrator.py`, `utils.py`), section-formatter argument types are bridged with casts / corrected annotations (`report.py`, `campaign.py`, `lineage.py`, `orchestrator.py`), a `LineageGraph.metadata` annotation and a forest-mode `metrics` None-guard were tightened, and small nits were cleared (unused `type: ignore`, missing `var_map` annotation, `dedent` argument, `driver` `main()` call). No runtime behavior changes. By @devdanzin.
- `--session-fuzz` no longer silently scores every child 0.0 when the `--target-python` interpreter can't import lafleur. Session fuzzing (and minimization) run the persistent driver *in the target* via `-m lafleur.driver`; if the target is a raw CPython build with no lafleur on its path, the import failed, the session produced no coverage, and every child scored 0.0 with no explanation. lafleur's package root is now prepended to the driver subprocess's `PYTHONPATH` so any target interpreter works, and `verify_target_capabilities` performs a loud preflight check (when `--session-fuzz` is set) that aborts with an actionable message if the target still can't import `lafleur.driver`, by @devdanzin.
...
- `CoroutineStateCorruptor` no longer injects `import sys` into the harness function body. A function-local `import sys` makes `sys` local to the whole harness, so every `sys.stderr` coverage marker that runs before it raised `UnboundLocalError` — killing the child before the JIT warmed and scoring it 0.0. The injected imports also accumulated across generations (`FuzzerSetupNormalizer` did not strip them), poisoning saved corpus files. The mutator now injects only `import asyncio` (which the coroutine needs; `sys` is already module-global via the boilerplate), and `FuzzerSetupNormalizer` now also strips `import sys` so already-poisoned corpus files self-heal on their next mutation. A real run had ~20% of children wasted on this; a reproduction confirms the fix. By @devdanzin.
- `BlockTransposerMutator` now only moves a statement block that is *independent* of the rest of the harness body. It previously relocated a random 3-statement block to a random position with no regard for name dependencies, so it frequently placed a variable/class/function use before its definition — raising `UnboundLocalError` at runtime and wasting the child (0.0 score). The mutator now skips a move when the block binds a name the rest loads (or loads a name the rest binds), which is safe-by-construction, by @devdanzin.
- The harness invocation is now wrapped in `try/except` for all seed families. Only the `uop` family wrapped its call (`for ...: try: harness() except Exception: break`); the `bug_pattern` family wrapped the harness *body* but invoked it bare, and `synthesize` had no wrapping — so ordinary mutation breakage (use-before-def, `TypeError`, `IndexError`) escaped to module level and exited the process 1. `bug_pattern`/`synthesize` seeds now emit a wrapped call, and `FuzzerSetupNormalizer` wraps any bare top-level `uop_harness_*()` call so existing corpus lineages self-heal. On a reproduction over the real corpus these two fixes together raised the clean-run rate from 58% to 93% (eliminating all escaping `UnboundLocalError`/`TypeError`/`IndexError`), by @devdanzin.
- `bug_pattern` and `synthesize` seeds now name their harness `uop_harness_f<N>` instead of `jit_harness_*`/`synth_harness_*`. The mutators, `_get_nodes_from_parent`, and `minimize.py` all key on `uop_harness`, so those families were previously never mutated — selecting one as a parent logged "No harness function found" and wasted the session (~57% of the corpus, given the family weights), by @devdanzin.
...
- Added `save_json_file()` and `append_jsonl()` utilities to `utils.py`, migrating 12 JSON write/append call sites across 8 modules for consistent indent=2, sort_keys, encoding, and error handling, by @devdanzin.
- Added explicit `encoding="utf-8"` to all `Path.read_text()`/`write_text()` calls, standardized `mkdir(parents=True, exist_ok=True)`, and routed `[!]` warnings to stderr in core modules, by @devdanzin.
