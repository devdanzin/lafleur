...
### Fusil
An external generative fuzzer used by lafleur to produce initial [Seed](#seed) files. Lafleur calls fusil with JIT-specific options to generate [Test Cases](#test-case) with hot loops suitable for triggering JIT compilation. See [Seed Generation and Management](./11_seeding.md) for the full seeding workflow and fusil invocation details.
...
### Seed
An initial [Test Case](#test-case) that forms the starting point for evolutionary exploration. Seeds are either generated by [Fusil](#fusil) or provided manually. They have no [Parent](#parent) ([Lineage Depth](#lineage-depth) 0) and are marked with `is_seed: True` in [Corpus](#corpus) metadata. See [Seed Generation and Management](./11_seeding.md) for how to create and manage seeds.
